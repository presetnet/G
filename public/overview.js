...
const groups = [["Geoff","geoff.","https://www.geoff.ai/"],["StackNet","stacknet.","https://stacknet.magma-rpc.com/health"],["TRIX","trix.","https://trix.market/"],["DIBZI","dibzi.","https://dibzi.ai/"],["SIM","sim.","https://sim.tech/"],["AZY","azy.","https://azy.life/"],["MAGMA","magma.","https://opensea.io/collection/the-simulation-958481099"]];
...
export function renderOverview(latest, events = []) {
  ...
  document.getElementById("quickFacts").innerHTML = `<article><h3>DIBZI auctions</h3><p>${usable ? `${esc(d.activeNames)} active · ${esc(d.namesTotal)} total including ${esc(d.cashtagNames)} cashtags` : "Values unavailable"}</p><p>${highest ? link(`https://dibzi.ai/name/${encodeURIComponent(highest.name)}`, `${highest.amountSol} SOL · ${highest.name}`) : "Highest bid unavailable"}</p><small>Observed ${age(d?.checkedAt)} · not a 24h sales total</small></article><article><h3>TRIX activity</h3><p>${link("https://trix.market/","Inspect market")}</p><small>Market: ${evidenceState(sources["trix.market"])} · ${age(sources["trix.market"]?.checkedAt)}<br>Generations: ${evidenceState(sources["trix.geoff"])} · ${age(sources["trix.geoff"]?.checkedAt)}</small><p>Records and receipts in the full desk below.</p></article><article><h3>SIM · sim.tech</h3><p>${link("https://sim.tech/","Inspect sim.tech")}</p><small>Site v${esc(sources["sim.site"]?.siteVersion ?? "?")} · ${sources["sim.site"]?.configured === true ? "X session reported" : "no X session"}<br>${sources["sim.chain"]?.simSupply != null ? `SIM supply ${sources["sim.chain"].simSupply.toLocaleString("en-US", { maximumFractionDigits: 0 })}` : "SIM supply unread"} · ${age(sources["sim.chain"]?.checkedAt)}<br>SOL since launch ${sources["sim.chain"]?.solDepositsSol != null ? `${sources["sim.chain"].solDepositsSol.toLocaleString("en-US", { maximumFractionDigits: 4 })} SOL` : "unread"} · ETH rail (Sepolia, instructed) ${sources["sim.eth"]?.ethDepositsEth != null ? `${sources["sim.eth"].ethDepositsEth.toLocaleString("en-US", { maximumFractionDigits: 4 })} ETH` : "unread"} · ETH mainnet (on-chain) ${sources["sim.ethm"]?.ethDepositsEth != null ? `${sources["sim.ethm"].ethDepositsEth.toLocaleString("en-US", { maximumFractionDigits: 4 })} ETH` : "unread"}<br>Deadline ${sources["sim.front"]?.deadline ? esc(sources["sim.front"].deadline.slice(0, 10)) : "not published"}</small><p>Payment rail, on-chain activity, deposit totals, and the countdown in the desk below.</p></article><article><h3>AZY · azy.life</h3><p>${link("https://azy.life/","Inspect azy.life")}</p><small>SOL intake ${sources["azy.chain"]?.azySolDepositsSol != null ? `${sources["azy.chain"].azySolDepositsSol.toLocaleString("en-US", { maximumFractionDigits: 4 })} SOL` : "unread"} · ${sources["azy.chain"]?.azyDepositCount != null ? `${sources["azy.chain"].azyDepositCount} deposits` : "—"} · ${sources["azy.chain"]?.azyUniquePayers != null ? `${sources["azy.chain"].azyUniquePayers} payers` : "—"}<br>Campaign-only SOL walk · custody ${esc(sources["azy.chain"]?.destWallet ?? "…")} · last ${age(sources["azy.chain"]?.destLatestAt)}</small></article><article><h3>MAGMA · The Simulation</h3><p>${link("https://opensea.io/collection/the-simulation-958481099","Inspect OpenSea")} · ${link("https://eth.blockscout.com/token/0xc3706195ff60658585b58716717ee7acc5ebca60","Blockscout")} · ${link("https://etherscan.io/token/0xc3706195ff60658585b58716717ee7acc5ebca60","Etherscan")}</p><small>ERC-721 from the simulation event · minted ${sources["magma.eth"]?.magmaSupply != null ? `${sources["magma.eth"].magmaSupply.toLocaleString("en-US", { maximumFractionDigits: 0 })}` : "?"} / ${sources["magma.eth"]?.magmaHolders != null ? `${sources["magma.eth"].magmaHolders.toLocaleString("en-US", { maximumFractionDigits: 0 })} holders` : "?"} / ${sources["magma.eth"]?.magmaTransfers != null ? `${sources["magma.eth"].magmaTransfers.toLocaleString("en-US", { maximumFractionDigits: 0 })} transfers` : "?"}${sources["magma.eth"]?.magmaMintedSincePrev > 0 ? `<br>+${sources["magma.eth"].magmaMintedSincePrev.toLocaleString("en-US", { maximumFractionDigits: 0 })} minted since last check · active now` : ""}<br>Observed ${age(sources["magma.eth"]?.checkedAt)} · on-chain counters, not wallet state</small></article>`;
  const seen = new Set();
