...
function fixture() {
  ...
  sources['trix.fee.config'] = { ...base, feeBps: 100, feeWallet: 'fixture-provider', treasuryWallet: 'fixture-treasury' };
   sources['dibzi.names'] = { ...base, source: 'dibzi.names', sourceUrl: 'https://dibzi.ai/api/names', configUrl: 'https://dibzi.ai/api/config', programId: 'fixture-dibzi-program', activeNames: 3, soldNames: 0, namesTotal: 3, totalBids: 3, uniqueWallets: 2, totalBidSol: 1.234, activeBoardSol: 2.234, recentBidCount60m: 2, bidsPerMinute60m: .033, namesReported: 3, nameSampleTruncated: false, highestBidSol: 1, names: [{ name: 'jimmy', amountSol: .8, leader: 'A'.repeat(32), bids: [] }, { name: 'trix', amountSol: .434, leader: 'B'.repeat(32), bids: [] }, { name: '$hoe', amountSol: 1, leader: 'A'.repeat(32), endsAt: at, settled: false, bids: [] }], topWallets: [{ rank: 1, username: 'jimmy', wallet: 'A'.repeat(32), totalBidSol: .8, bids: 2, leading: 1 }, { rank: 2, wallet: 'B'.repeat(32), totalBidSol: .434, bids: 1, leading: 1 }], recentBids: [{ name: 'jimmy', amountSol: .8, at, signature: 'dibzi-signature' }, { name: 'trix', wallet: 'B'.repeat(32), amountSol: .434, at, signature: 'dibzi-signature-2' }] };
...
async function main() {
  ...
  try {
    ...
    assert.match(await page.locator('#dibziSummary').textContent(), /reported bids\/min/);
